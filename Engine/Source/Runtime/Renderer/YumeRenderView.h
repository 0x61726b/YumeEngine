...
namespace YumeEngine
{
	...
	class YumeAPIExport YumeRenderView : public YumeBase
	{
		friend void CheckVisibilityWork(const WorkItem* item,unsigned threadIndex);
		friend void CheckVisibilityExp(const WorkItem* item,unsigned threadIndex);
	...
	private:
		...
		void QuantizeDirLightShadowCamera(YumeCamera* shadowCamera,YumeLight* light,const IntRect& shadowViewport,const BoundingBox& viewBox);
		bool IsShadowCasterVisible(YumeDrawable* drawable,BoundingBox lightViewBox,YumeCamera* shadowCamera,const Matrix3x4& lightView,const Frustum& lightViewFrustum,const BoundingBox& lightViewFrustumBox);
		...
		YumeRHI* graphics_;
		WeakPtr<YumeRenderer> renderer_;
		YumeScene* scene_;
		Octree* octree_;	
		YumeCamera* camera_;	
