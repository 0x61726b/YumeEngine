...
namespace YumeEngine
{
	...
	void ShadowsDemo::Start()
	{
		...
		overlay_->GetBinding("SampleName")->SetValue("Shadows");
		...
		light1_ = scene_->CreateChild("plight");
		light1_->SetPosition(Vector3(0,1,-3.7f));
		light1_->SetRotation(Quaternion(45));
		YumeLight* plight = light1_->CreateComponent<YumeLight>();
		plight->SetLightType(LIGHT_SPOT);
		plight->SetColor(YumeColor(0.5f,0.5f,1,1));
		plight->SetFov(60.0f);
		plight->SetShadowBias(BiasParameters(0.00025f,0.5f));
		...
		test3->SetPosition(Vector3(0,0.5f,0));
